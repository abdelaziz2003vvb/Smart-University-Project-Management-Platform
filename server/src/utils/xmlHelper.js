...
exports.buildProjectXML = (project) => {
  const builder = new xml2js.Builder({ rootName: 'project', xmldec: { version: '1.0', encoding: 'UTF-8' } });
  const projectData = {
    id: project._id.toString(),
    title: project.title,
    description: project.description,
    status: project.status,
    student: project.studentId ? { id: project.studentId._id.toString(), name: project.studentId.name, email: project.studentId.email } : {},
    teacher: project.teacherId ? { id: project.teacherId._id.toString(), name: project.teacherId.name, email: project.teacherId.email } : {},
    tasks: { task: project.tasks.map(task => ({ id: task._id.toString(), title: task.title, description: task.description || '', deadline: task.deadline ? task.deadline.toISOString() : '', progress: task.progress, status: task.status })) },
    grade: project.grade || '',
